...
describe('focusBounds — HOUR floor 산식', () => {
  ...
  it('1. nowMs ≤ 0 (SSR snapshot) → 전체 (0~24h)', () => {
    expect(focusBounds(720, 0)).toEqual({slotMinTime: '00:00:00', slotMaxTime: '24:00:00'});
    expect(focusBounds(240, -1)).toEqual({slotMinTime: '00:00:00', slotMaxTime: '24:00:00'});
  });

  // 핵심 — HOUR floor 분 절삭 검증 (대장 명시 사양)
  it('2. 9:35 + 4h → [8:00, 12:00] (분 절삭)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 9, 35))).toEqual({
  ...
  it('3. 9:00 + 4h → [8:00, 12:00] (분 0 — boundary)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 9, 0))).toEqual({
      slotMinTime: '08:00:00',
      slotMaxTime: '12:00:00'
    });
  });
  ...
  it('4. 9:59 + 4h → [8:00, 12:00] (분 59 — 같은 시간대 그대로)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 9, 59))).toEqual({
      slotMinTime: '08:00:00',
      slotMaxTime: '12:00:00'
    });
  });
  ...
  it('5. 10:00 + 4h → [9:00, 13:00] (sliding window 자동 이동)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 10, 0))).toEqual({
  ...
  it('6. 12:00 + 4h(240) → [11:00, 15:00]', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 12))).toEqual({
  ...
  it('7. 12:00 + 6h(360) → [11:00, 17:00]', () => {
    expect(focusBounds(360, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('8. 12:00 + 8h(480) → [11:00, 19:00]', () => {
    expect(focusBounds(480, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('9. 12:00 + 10h(600) → [11:00, 21:00]', () => {
    expect(focusBounds(600, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('10. 12:00 + 12h(720 default) → [11:00, 23:00]', () => {
    expect(focusBounds(720, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('11. 12:00 + 16h(960) → [11:00, 27:00] (다음날 03:00 영역)', () => {
    expect(focusBounds(960, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('12. 12:00 + 20h(1200) → [11:00, 31:00]', () => {
    expect(focusBounds(1200, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('13. 12:00 + 24h(1440 max) → [11:00, 35:00]', () => {
    expect(focusBounds(1440, ymdms(2026, 5, 6, 12))).toEqual({
      slotMinTime: '11:00:00',
  ...
  it('14. 21:40 + 12h(default) → [20:00, 32:00] (실측 보고 사례 · 자정 넘는 view)', () => {
    expect(focusBounds(720, ymdms(2026, 5, 6, 21, 40))).toEqual({
  ...
  it('15. 23:30 + 12h → [22:00, 34:00] (다음날 10시까지 연속 view)', () => {
    expect(focusBounds(720, ymdms(2026, 5, 6, 23, 30))).toEqual({
  ...
  it('16. 23:59 + 24h → [22:00, 46:00] (max range · 자정 직전)', () => {
    expect(focusBounds(1440, ymdms(2026, 5, 6, 23, 59))).toEqual({
      slotMinTime: '22:00:00',
  ...
  it('17. 0:00 + 4h → [00:00, 04:00] (h=0 → startMin=-60 → clamp 0)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 0, 0))).toEqual({
  ...
  it('18. 0:30 + 4h → [00:00, 04:00] (h=0 · 분 절삭 + startMin clamp 0)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 0, 30))).toEqual({
      slotMinTime: '00:00:00',
      slotMaxTime: '04:00:00'
    });
  });
  ...
  it('19. 1:00 + 4h → [00:00, 04:00] (h=1 → startMin=0 정확)', () => {
    expect(focusBounds(240, ymdms(2026, 5, 6, 1, 0))).toEqual({
      slotMinTime: '00:00:00',
      slotMaxTime: '04:00:00'
    });
  });
  ...
  it('20. 17:30 + 8h → [16:00, 24:00] (24:00 boundary)', () => {
    expect(focusBounds(480, ymdms(2026, 5, 6, 17, 30))).toEqual({
